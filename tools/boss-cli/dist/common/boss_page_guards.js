...
const SHOULD_ALLOW_CONSOLE_CLEAR = process.env.BOSS_BROWSER_ALLOW_CONSOLE_CLEAR === 'true' ||
 ...
 */
const SHOULD_ALLOW_VERBOSE_CONSOLE = process.env.BOSS_BROWSER_ALLOW_VERBOSE_CONSOLE === 'true' ||
...
const REPORT_REQUEST_RE = /(?:logapi(?:-dev)?\.(?:zhipin|weizhipin)\.com\/dap\/api\/json|apm-fe(?:-qa)?\.(?:zhipin|weizhipin)\.com\/wapi\/zpApm\/|warlock\.zhipin\.com\/wapi\/warlock\/|shink\.zhipin\.com\/wapi\/dapCommon\/json)/i;
const RISK_NAVIGATION_RE = /about:blank|\/web\/common\/(?:403|nonsupport)\.html|\/web\/user\/safe\/verify|\/web\/passport\/(?:zp\/(?:403|verify|security)\.html|cm\/(?:403|verify|security-check)\.html)/i;
const PAUSED_LABEL = {
    report: 'report:204',
...
function isReportRequestUrl(url) {
    return REPORT_REQUEST_RE.test(url);
}
function isRiskNavigationUrl(url) {
    return RISK_NAVIGATION_RE.test(url);
}
function classifyPausedRequest(url) {
    if (REPORT_REQUEST_RE.test(url))
        return 'report';
    if (RISK_NAVIGATION_RE.test(url))
        return 'risk_navigation';
 ...
 */
function logToPageConsole(cdp, message) {
    void cdp
...
function buildNoContentResponseHeaders(headers) {
    const origin = readRequestHeader(headers, 'origin') ?? 'https://www.zhipin.com';
    const requestedHeaders = readRequestHeader(headers, 'access-control-request-headers') ??
...
const browsersWithTargetGuard = new WeakSet();
const pagesWithInitGuard = new WeakSet();
const pagesWithNavigationGuard = new WeakSet();
const pagesWithRequestGuard = new WeakSet();
async function ensurePageInitGuard(page) {
    if (pagesWithInitGuard.has(page))
        return;
...
function ensurePageNavigationGuard(page) {
    if (pagesWithNavigationGuard.has(page))
        return;
...
async function ensurePageRequestGuard(page) {
    if (pagesWithRequestGuard.has(page))
        return;
...
export async function installBossPageGuards(page) {
    if (page.isClosed())
        return;
...
async function installTargetPageGuards(target) {
    if (target.type() !== 'page')
        return;
...
export async function installBossBrowserPageGuards(browser) {
...
//# sourceMappingURL=boss_page_guards.js.map