import { randomIntInclusive, selectAllModifierKey, SEND_AFTER_ENTER_MS, SEND_INPUT_CLICK_MS, SEND_TYPING_GAP_MS, sleepRandom, typeTextWithRandomKeyDelay, } from '../browser/index.js';
...
async function sendTextOnCurrentPage(page, messageText, options = {}) {
...
async function assertExactCurrentChat(page, name, job) {
    if (!isBossChatIndexUrl(page.url())) {
    ...
    if (!state.valid) {
        throw new Error(`精确会话验证失败：目标“${name} / ${job}”，当前“${state.selectedName || state.detailName || 'unknown'} / ${state.communicationPosition || 'unknown'}”。`);
    }
}
async function countExactOutgoingMessage(page, message) {
...
async function waitForNewExactOutgoingMessage(page, message, previousCount) {
    const messageLiteral = JSON.stringify(message);
...
export async function runSendChatMessageSequence(options) {
