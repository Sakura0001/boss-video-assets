...
| `/web/chat/recommend` | 推荐候选人列表 | `boss recommend [岗位关键字] [--refresh]`、`boss preview <姓名>`、`boss greet <姓名>` | `recommend` 会先检查当前位置，不在该页时直接进入；`--refresh` 会随机等待 1–2 秒后显式刷新推荐页并读取新列表；`preview` / `greet` 要求当前已在该页且列表已加载。 |
