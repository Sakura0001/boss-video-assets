...
| `/web/chat/index` | 沟通列表、候选人会话页 | `boss list`、`boss list --unread`、`boss chat <姓名>`、`boss chat [姓名] --index <序号> [--unread]`、`boss send`、`boss send-sequence <姓名> --job <岗位> --messages-json <JSON数组> --json`、`boss action` | `list` / `chat` 会先检查当前位置，不在该页时直接进入；`chat --index` 使用 `boss list` 输出的 1-based 序号，带 `--unread` 时使用 `boss list --unread` 输出的序号；同时提供姓名时会校验序号对应候选人姓名，适合处理同名候选人；`send` / `action` 要求当前会话已打开。`send-sequence` 只打开一次精确会话，每条发送前核对当前姓名和岗位，并逐条读回验证三条消息。 |
| `/web/chat/recommend` | 推荐候选人列表 | `boss recommend [岗位关键字] [--refresh] [--json]`、`boss preview <姓名>`、`boss greet <姓名> [--id <候选人ID>] [--job <岗位关键字>] [--json]` | `recommend` 始终用浏览器鼠标事件点击主壳左侧推荐入口，让 Boss 前端路由挂载 `recommendFrame`；不得用 DOM `.click()` 代替。当前岗位标签已经匹配关键词时直接复用，不重复打开岗位下拉；`--refresh` 会随机等待 1–2 秒后显式刷新推荐页并读取新列表；`--json` 输出去重后的稳定候选人 ID、卡片字段，以及右侧 `.edu-exps` 中每条教育经历的起止年份、学校、专业和学历。`greet --id` 只点击 ID 精确命中的推荐卡片；`preview` / `greet` 要求当前已在该页且列表已加载。 |
