...
| `/web/chat/recommend` | 推荐候选人列表 | `boss recommend [岗位关键字] [--refresh] [--json]`、`boss preview <姓名>`、`boss greet <姓名> [--id <候选人ID>] [--job <岗位关键字>] [--json]` | `recommend` 始终通过主壳左侧推荐入口进入，让 Boss 前端路由挂载 `recommendFrame`；当前岗位标签已经匹配关键词时直接复用，不重复打开岗位下拉；`--refresh` 会随机等待 1–2 秒后显式刷新推荐页并读取新列表；`--json` 输出去重后的稳定候选人 ID、卡片字段，以及右侧 `.edu-exps` 中每条教育经历的起止年份、学校、专业和学历。`greet --id` 只点击 ID 精确命中的推荐卡片；`preview` / `greet` 要求当前已在该页且列表已加载。 |
