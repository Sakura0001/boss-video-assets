...
| `/web/chat/recommend` | 推荐候选人列表 | `boss recommend [岗位关键字] [--refresh] [--json]`、`boss preview <姓名>`、`boss greet <姓名> [--id <候选人ID>] [--job <岗位关键字>] [--json]` | `recommend` 会先检查当前位置，不在该页时直接进入；`--refresh` 会随机等待 1–2 秒后显式刷新推荐页并读取新列表；`--json` 输出去重后的稳定候选人 ID、卡片字段，以及右侧 `.edu-exps` 中每条教育经历的起止年份、学校、专业和学历。`greet --id` 只点击 ID 精确命中的推荐卡片；`preview` / `greet` 要求当前已在该页且列表已加载。 |
