...
当前给普通用户使用的运行范围只有：筛选合格候选人、打招呼、发送两条知识库
...
`skills/boss-zhaopin/references/` 读取学校、专业和两条主动招呼话术；
...
该模式只读取并校验配置，输出 `messageCount: 2`；不获取执行锁，不初始化或清理
本地状态，也不调用登录、推荐、在线简历预览、消息发送或其他 Boss 操作。
...
在当前会话中按知识库原文逐条发送并验证两条消息，不主动索要附件简历。
每次发送前仍会核对姓名和沟通岗位，但不会反复退出、重进聊天页。两条消息
全部验证后记录为等待候选人说明投递状态。每位候选人操作前随机等待 1–2 秒，
进度输出包含该候选人的完整流程耗时。达到150、超出09:00–21:00、达到
候选人检查上限，或遇到登录、验证码、风控、精确会话和发送异常时停止。
进度中的 `selectionSeconds` 是筛选耗时，`workflowSeconds` 是招呼到两条
消息验证完成的耗时，`cycleSeconds` 是该候选人的完整周期耗时。

不要与另一台电脑、第二个终端或任何其他 Boss 自动化执行器同时运行。

用户只需要运行上述 Python 命令；候选人筛选、推荐页刷新、精确 ID 招呼、
聊天页切换、两条消息发送、结果验证和状态记录均由脚本自动编排。
