...
5. 两条岗位介绍发送完后，主动索要附件简历：`boss send --text "方便的话辛苦发我一份附件简历，我帮你进一步看下和数据库方向的匹配度。" --request-resume`。
6. 如果候选人先发起附件简历请求，必须先执行 `boss action agree-resume` 接收简历，并重新打开会话确认 `简历获取状态: 已获取`，再进入后续投递状态确认、投递引导或微信承接。
