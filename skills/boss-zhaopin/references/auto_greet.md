...
5. 只处理资料明确的 2027 届、本科或研究生、目标学校、且专业语义上能明确判断为计算机类或与 `school_policy.yaml` 的 `allowed_majors` 某一方向相近的候选人；目标学校全称或批准别名必须作为独立字段完整命中，更长院校名称的包含关系不得通过。学校自定义的近似专业名称可以通过，不要求技术经历。明显无关或无法判断时不通过。
