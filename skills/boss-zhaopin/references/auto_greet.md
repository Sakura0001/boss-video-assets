...
5. 先移除求职期望中的全部空白并转为小写，再对固定硬编码列表做子串匹配；固定列表仅为六项：算法、通信、硬件、前端、unity、电气。命中时内部原因 `expectation_blocked`，不回复、不打招呼、不发送后续三条消息，完整求职期望不得写入本地状态或 Git。为空或未命中时，继续毕业年份、学历、学校、专业和去重门槛；随后从页面右侧结构化教育经历读取每一条学校、专业和学历。只处理资料明确的 2027 届、本科或研究生、至少一所教育经历学校命中目标名单、且当前学历对应专业语义上能明确判断为计算机类或与 `school_policy.yaml` 的 `allowed_majors` 某一方向相近的候选人。硕士或博士显示多所学校时，任意一所精确命中即可；目标学校全称或批准别名必须作为独立字段完整命中，更长院校名称的包含关系不得通过。学校自定义的近似专业名称可以通过，不要求技术经历。确定性脚本仅使用 `allowed_majors` 直接命中或 `major_aliases` 显式映射，不得自由猜测；明显无关、未列入映射或无法判断时不通过。
