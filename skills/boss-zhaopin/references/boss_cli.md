...
`--request-resume` attempts to trigger Boss's attachment-resume request after sending. Boss may reject it until both sides have sent at least one message. This standalone capability is for separately authorized manual flows only; the proactive runner never uses it.
