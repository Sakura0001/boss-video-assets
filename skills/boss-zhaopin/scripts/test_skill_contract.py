...
class SkillContractTest(unittest.TestCase):
    ...
    def test_proactive_greeting_has_three_exact_machine_readable_sections(self):
        ...
        expected = (
            ...
            (
                "投递方式与流程提示",
                "在华为，投递一个好的部门远好于投递一个非常热门的业务，请同学慎重考虑投递的部门，当前如果还没进入简历筛选环节或者投递的不是云软件研发部，还可以撤销原投递转投我们部门，投递方式如下，AI应用工程师投递：校园招聘->校招生职位->AI应用工程师->AI技术应用/AI系统软件->选择工作地点->部门意向选择“ICT BG” “云软件研发部” 或者 软件开发工程师投递：校园招聘->校招生职位->软件开发工程师->通用软件/数据库->选择工作地点->部门意向选择“ICT BG” “云软件研发部” 两个岗位仅在笔试环节不同，可以选择自己擅长的笔试方向投递，投递后我会发你一些笔试/测评攻略。并加我微信后续跟踪流程。",
            ),
        )
        actual = []
    ...
    def test_candidate_filter_is_2027_any_visible_school_stem_and_silent_on_unknown(self):
        ...
        self.assertNotIn("电子科学与技术", policy)
        self.assertNotIn("集成电路工程", policy)
        self.assertNotIn("数字媒体技术", policy)
        self.assertNotIn("微电子学", policy)
