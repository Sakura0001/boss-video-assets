...
如果候选人表现出意向，自动回复流程应转入 `workflows/candidate_conversion.md`：先索要附件简历；如果候选人已经发送简历，或围绕岗位优势、组织归属、地点、流程等关键问题沟通差不多了，则发送微信交换入口，并要求候选人主动添加我的微信。
...
8. 如果候选人表现出意向，优先执行索要简历流程：`boss send --text "方便的话辛苦发我一份附件简历，我帮你进一步看下和数据库方向的匹配度。" --request-resume`。
9. 如果候选人已经发送简历，或候选人已明确表达兴趣且沟通差不多，发送微信沟通邀请，再执行 `boss action wechat`。
10. 执行 `boss action wechat` 后，必须补充说明让候选人主动添加我的微信，因为账号一天不能主动添加太多同学，频繁添加容易异常。
