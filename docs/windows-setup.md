...
两条知识库消息和断点续跑都封装在 `scripts/greet_only.py` 中。
...
每位候选人的两条消息只打开一次精确会话，并在同一浏览器会话内逐条发送
和验证；两条均验证后记录为等待投递状态。程序不调用 Claude、不处理未读、
不处理到期跟进、不主动索要附件简历、不交换微信，也不自行生成消息。
...
- 平台点击“打招呼”产生的默认开场白由 Boss 账号配置决定；随后两条消息
